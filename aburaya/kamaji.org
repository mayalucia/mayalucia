...
the extractable interests that other processes (like relay-protocol)
...
- /Parent-level spirit/ (e.g. mayadev): broad context, many topics
...
The relay-protocol power, when invoked for reading, collects filter tags from two
sources:
