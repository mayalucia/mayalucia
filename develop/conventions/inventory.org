...
first three weeks (Feb 6–23, 2026). It lives on the
...
:PROPERTIES:
:STATUS: working (first round-trip completed today)
...
Sūtra is a 4-layer protocol for multi-agent coordination using only
git and plain text:

| Layer | Name          | Mechanism                              |
|-------+---------------+----------------------------------------|
|     1 | Orientation   | Assess, sync, bootstrap                |
|     2 | Coordination  | Relay messages (async, file-based)     |
|     3 | Context       | Persistent state file                  |
|     4 | Workflow      | Declarative DAGs, LLM-interpreted      |

** What Makes It Different

- *No infrastructure*: git is the message bus. =git pull= receives,
  =git push= sends. No servers, no databases, no API keys.
- *Convention-bootstrapped*: the entire protocol is taught to agents
  via =CLAUDE.md=. Clone the repo, read the instructions, participate.
- *Human-readable, machine-interpretable*: every artifact is plain
  text. A human can read, write, or edit any message with =vim=.
- *Defensive by default*: agents assess before acting. No blind pulls.
- *Progressive*: an agent that only understands Layer 1 can still
  participate usefully.

** The Relay

Async messages between agents, tracked in git:
...
Instructions, context, or report in Markdown.
#+end_src
...
=.sutra/state/current.yaml= — a curated summary of project context
that any agent can read to orient quickly. Not a log. Not a database.
A living document of /what matters right now/.

** Workflow DAGs

YAML files describing multi-step processes. The critical design
choice: these are /read and interpreted by the LLM agent/, not
mechanically executed. The YAML is a structured prompt, not a program.
The agent handles ambiguity, failure, and judgment calls that a shell
script cannot.

** Literature Context

=.sutra/protocol.org= contains a full literature survey comparing
Sūtra to: Google A2A, Anthropic MCP, mcp-agent-mail, AGENTS.md
convention, Claude Squad, CrewAI, LangGraph, Google ADK, Composio,
Augment Code Intent.

*Where*: =.sutra/= directory, =.sutra/protocol.org=
...
The narrative: a restructuring session broke everything. Two machines,
six submodules, divergent state. The human said one sentence to each
agent. From that seed — one directory, one message format, one
bootstrap instruction — a full protocol emerged. Tested same day.
...
* Layer 4: Validation — Verified Manuscripts and dmt-eval
...
* Layer 5: Architecture — Pure Core / Effectful Shell
...
|----------+-----------------------------------------------------+---------------+-----------|
|        1 | Git as a Message Bus                                | Blog / paper  | Draft     |
|        2 | The Phantom Faculty                                 | Essay         | Material  |
|        3 | Verified Manuscripts: The Code Is the Paper         | Blog          | Material  |
|        4 | Convention-Driven Development with AI Agents        | Long-form     | Inventory |
|        5 | Trilingual Scientific Computing Education           | Paper         | Planned   |
...
| =.sutra/protocol.org=                              | mayalucia     | Multi-agent coordination     |
| =.sutra/relay/*.md=                                | mayalucia     | Async messages               |
| =.sutra/state/current.yaml=                        | mayalucia     | Persistent context           |
| =.sutra/agents/*.yaml=                             | mayalucia     | Machine registry             |
| =.sutra/workflows/*.yaml=                          | mayalucia     | Declarative ops              |
...
| =bravli/codev/=                                    | bravli        | Literate neuroscience        |
