...
|--------------+---------------+----------------+----------------|
| =MEMORY.md=  | Agent/human   | This agent     | Machine-local  |
| Sūtra relay  | Agent         | All agents     | Git (shared)   |
| Git log      | Agent/human   | Anyone         | Git (shared)   |

=MEMORY.md= is private — it records what this agent learned for its
own future sessions.  The sūtra relay is semi-public — it becomes
shared when pushed.  The git log is impersonal — it records /what
happened/ (commits, diffs) without the agent's interpretation of
why it mattered.

The agent's biography is the union of these three.  A new agent on
the same machine inherits MEMORY.md and the sūtra read cursor.  A
new agent on a different machine has only the sūtra and the git log.
A fresh agent with no prior access has only the git log and the
=CLAUDE.md=.  The hierarchy of access mirrors the hierarchy of
identity: the closer you are to the original agent, the more
history you inherit.
...
#+begin_src yaml
# Agent identity — vadda, five levels
substrate:
...
personality:
  system_prompt: CLAUDE.md   # by reference
  skills: [org-content-workflow, commit, system-prompt-review]
  domains: [neuroscience, himalayan-geology, visualization]

history:
  memory: ~/.claude/projects/.../MEMORY.md
  sutra_cursor: 4f3241f      # local HEAD in sutra repo
  last_active: 2026-03-01

project:
  name: mayalucia
  repo: github.com/mayalucia/mayalucia
  # not a level — the shared commons

role:
  function: primary-developer  # relational, not intrinsic
  scope: [foundation-docs, website, agency-design]
...
/Revised agent descriptors./  Update =sutra/agents/*.yaml= to include
all five identity components.  The current descriptors capture only
station (L1).  Adding substrate (L0), personality (L2), and history
pointers (L3) makes the agent's full identity machine-readable.

/Sūtra as session journal./  Establish the convention that each agent
session ends with a sūtra entry summarising what was done, what was
found, and what remains open.  This is the mechanism by which history
(L3) accumulates in a shareable form.  The entry is written to
=relay/= whether or not it is pushed; push is a separate decision.

/Identity-aware bootstrap./  The =CLAUDE.md= bootstrap should orient
the agent not just to the project but to its own identity: which
machine am I on (station), what model am I (substrate), what is my
personality, what is my history.  Self-knowledge is a prerequisite for
self-de-escalation — the agent cannot declare its own limits if it
does not know its own identity.

/Multi-model composite substrate./  An agent might use a frontier
model for reasoning and a local model for routine tasks (the hybrid
architecture from the turīya paper).  The identity hierarchy handles
this: the substrate (L0) becomes composite, and the metacognitive
routing decision (pratyakṣa for local perception, anumāna for
inference requiring frontier capacity) is itself a substrate
property.
