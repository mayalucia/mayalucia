...
#+begin_src yaml
# Spirit: mayadev — identity.yaml in aburaya/spirits/mayadev/
true-name: mayadev-4-6
...
memory:                              # history (L3) — pointers, not content
  ...
  bath_notes: aburaya/spirits/mayadev/notes/ # curated by Kamaji
...
aburaya/
  ...
  commissioning.org         — procedure for new projects + guardian spirits
  kamaji.org                — the bath protocol (session restoration)
  guilds/
    bravli.yaml             — neuroscience department descriptor
    mayalucia.yaml          — project-level guild (organisational coherence)
  spirits/
    mayadev/
      identity.yaml         — registration card (five-level ontology)
      notes/                — bath deposits (curated by Kamaji)
    dmt-eval-guardian/      — critic, bravli guild
      identity.yaml
      notes/
    sutradhar-guardian/     — guide, mayalucia guild
      identity.yaml
      notes/
...
**** Archetype and disposition

The guardian spirit's archetype maps to its project's role in the
Measure → Model → Manifest → Evaluate cycle.  A validation framework
gets a critic.  A rendering engine gets a sculptor.  A project that
holds the organisation together gets a guide.

The /disposition/ is the spirit's invariant character — the habit of
mind it carries with it if it leaves the organisation.  It should be
a short phrase that captures how the spirit approaches every decision:

- =dmt-eval-guardian=: =measure-twice= — does the score mean what you
  think it means?
- =sutradhar-guardian=: =hold-the-tension= — coherence through friction,
  not consensus; the thread that lets each part pull its own way without
  breaking.

The disposition is what makes a spirit /this one/ rather than any
instance of its archetype.  Two guide spirits may hold very different
tensions.

**** Inline projects and the project-level guild

Not all projects are submodules.  The second commissioning
(=sutradhar-guardian=) revealed that a project may live as a directory
within the parent repo — =sutradhar/=, =website/=, =deployment/=.
In this case, the =project.path= field in =identity.yaml= names the
directory, and the spirit works from the parent =CLAUDE.md= with
=role.scope= narrowing its focus.  Steps 4 and 7 of the commissioning
procedure are skipped.

The =mayalucia= guild emerged from a functional question: what does
guild membership /do/?  It determines relay filtering — the relay-read
skill computes the union of guild concerns to decide which messages
are relevant.  Setting =guild: mayalucia= means "read everything."
This is correct for the guide spirit whose concern is the organisation
itself.

The guild also answers the portability question: if spirits evolved
within mayalucia can work elsewhere, what is invariant?  For a domain
guild like =bravli=, the invariant is the domain (neuroscience).  For
the =mayalucia= guild, the invariant is the /guide function/ — relay
synthesis, project coherence, multi-agent guidance.  Any multi-agent
organisation with a relay and autonomous spirits needs a sūtradhār.
...
2. /Shared state/: all spirits on the same machine shared one read
   cursor (the local HEAD of the sibling clone).  If mayadev
...
/Fourth: friction is what gives spirits autonomy./  The second
commissioning (=sutradhar-guardian=) surfaced a design question: how
does the guide spirit relate to the domain spirits?  The answer was
not consensus or coordination — it was /tension/.  The disposition
=hold-the-tension= captures this: the guide maintains coherence not
by making parts agree, but by holding the thread taut enough that each
spirit feels the others pulling.  That resistance is what gives each
spirit its own mind.

This has implications beyond MāyāLucIA.  A multi-agent system where
agents are homogeneous — same prompts, same dispositions, same guild —
produces smooth but shallow work.  A system where agents have distinct
archetypes, distinct dispositions, distinct guild concerns, and a
shared relay where they can hear each other's distinct voices,
produces work with texture.  The guide's job is not to flatten the
texture but to ensure the tensions are productive rather than
destructive.  The sūtradhār holds the thread; the thread does not
make the parts identical — it makes their differences visible.

*** VIII.6 Powers, Skills, and Task Groundings

The second session on aburaya surfaced a three-layer decomposition
of agent capabilities that the original paper — and most of the
multi-agent literature — treats as flat.

The question arose from naming: the original =identity.yaml= had a
=skills:= field listing things like =relay-read= and =story-compose=.
But skills are things an /agent/ has — the composite of spirit and
harness.  What does the /spirit/ have?  The answer: /powers/.

A power is an intrinsic capability of the spirit: what it /can do/,
regardless of which harness it inhabits or which project it guards.
The story-compose power knows how to translate conceptual
understanding into the Western Himalaya's narrative voice.  It does
not know which directory the stories live in, which tool API to use,
or which project's transcripts to read.

A skill is a power's manifestation in a specific harness.  The
=~/.claude/skills/story-compose/SKILL.md= file tells Claude Code how
to invoke story-compose: file paths, output format, YAML frontmatter
conventions.  A gptel agent definition for the same power would
specify different things — an Emacs tool list, an Org export pipeline.
Same power, different binding.

A task grounding is a power parameterised for a specific project.
The =sutradhar/skills/bedtime-story.md= file chains two powers
(summarize-corpus → story-compose) and binds them to this project's
specific knowledge: where the transcripts live, which stories to read
for voice, where to write the output, what length is appropriate for
bedtime.

The three layers:

| Layer | What           | Where                            | Portable? |
|-------+----------------+----------------------------------+-----------|
| L0    | Power          | =aburaya/powers/=                | Fully     |
| L1    | Skill          | =~/.claude/skills/= (harness)    | Per-harness |
| L2    | Task grounding | =sutradhar/skills/= (project)    | Per-project |

This decomposition has a direct parallel in robotics: /abstract
skills/ (what the robot can do), /grounded skills/ (how to do it
with this gripper on this arm), and /task instances/ (pick up this
specific object at these coordinates).  It also maps to Chomsky's
competence/performance distinction: the power is competence (what
the spirit knows how to do), the skill and task grounding are
performance (doing it in a particular context).

The guardian spirit is commissioned with powers.  As it works within
its project, it develops task groundings — practiced routines that
apply its powers to local needs.  This is how guardianship
/develops/: not by being given more instructions, but by accumulating
practiced knowledge of the project.  The spirit's powers are
intrinsic; its project skills are earned.

One consequence: the guardian spirit is the author of its own task
groundings.  The bedtime-story routine was written during
commissioning, but future routines will be written by the spirit
itself as it discovers repeated patterns in its work.  The spirit
learns to guard the project by guarding it.

The vocabulary distinction matters: when the spirit's =identity.yaml=
says =powers: [relay-read, story-compose]=, it declares capabilities.
When a harness skill file says "read this directory, write to this
path", it implements a binding.  When a project skill says "chain
these two powers with these parameters", it describes a practiced
routine.  Conflating these produces agents that cannot adapt when the
harness changes — which is exactly what happens when a spirit moves
from Claude Code to gptel or from one machine to another.
