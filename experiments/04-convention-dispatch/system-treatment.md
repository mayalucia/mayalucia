...
When your task matches a type below, read the linked convention before starting:

| Task type | Convention document | Key constraint |
|-----------|-------------------|----------------|
| Survey a domain or ecosystem | `develop/ecological-surveys.org` | Three temporal layers (construction, configuration, unravelling) |
| Illustrate a survey with figures | `develop/illustrated-surveys.org` | Matplotlib, PEP 723, valley-native visual metaphors |
| Draft or modify a work package | `develop/work-packages.org` | Five required sections; `#+property: status` lifecycle |
| Compose a story | `develop/story-development.org` | Found-document framing; no fourth-wall leaks; `keys.org` translation table |
| Review a spirit's work | `develop/review-practice.org` | Structured review with evidence citations |
| Commission a new spirit | `aburaya/commissioning.org` | 10-step procedure; identity.yaml schema |
| Modify capabilities or powers | `aburaya/capabilities.org` | Three-layer decomposition (power / skill / body) |
| Set up a `.lab/` workspace | `develop/lab.org` | Gitignored, regenerable artefacts only |
| Plan collaboration or co-ownership | `develop/co-ownership.org` | Complementary-intelligence model |
| Design orchestration or dispatch | `develop/orchestration.org` | DAG-based dispatch plans |

If your task does not match any row, proceed without a convention — but note in your output that no convention was found.
